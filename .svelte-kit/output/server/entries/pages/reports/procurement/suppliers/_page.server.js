...
const load = async ({ url }) => {
  ...
  const leaderboardRaw = lbGrouped.map((r) => ({ supplierId: Number(r.supplierId), qty: Number(r._sum.qtyDelta ?? 0) })).filter((r) => r.qty > 0).sort((a, b) => b.qty - a.qty);
  ...
  if (filters.supplierId) {
    ...
    selectedSupplier = {
      id: supIdNum,
      name: supplierNameMap.get(supIdNum) || (await prisma.supplier.findUnique({ where: { id: supIdNum }, select: { name: true } }))?.name || `S${supIdNum}`
    };
    ...
    detailRows = detailGrouped.map((r) => ({
      mmaCode: r.mmaCode,
      shade: r.shade,
      size: r.size,
