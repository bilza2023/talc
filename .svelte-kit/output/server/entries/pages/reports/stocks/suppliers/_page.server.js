...
const load = async ({ url }) => {
  ...
  const leaderboardRaw = groupedLB.map((r) => ({ supplierId: Number(r.supplierId), qty: Number(r._sum.qtyDelta ?? 0) })).filter((r) => r.qty > 0).sort((a, b) => b.qty - a.qty);
  const supplierIds = leaderboardRaw.map((r) => r.supplierId);
  const supplierMap = supplierIds.length ? new Map(
  ...
  if (filters.supplierId) {
    ...
    selectedSupplier = {
      id: supIdNum,
      name: supplierMap.get(supIdNum) || (await prisma.supplier.findUnique({ where: { id: supIdNum }, select: { name: true } }))?.name || `S${supIdNum}`
    };
    const whereDetail = makeWhere(
      filters,
      /*includeSupplier*/
      true
    );
    ...
    detailRows = groupedDetail.map((r) => ({
      mmaCode: r.mmaCode,
      shade: r.shade,
      size: r.size,
  ...
  return {
    ...
    options: {
      ...
      shades,
      sizes,
      supplierIds: leaderboard.map((x) => String(x.supplierId))
      // use numeric string values
    },
    kpis: [
