...
function arrayDelegate(rows) {
  return {
    ...
    async findMany({ orderBy = [], skip = 0, take, select } = {}) {
      let out = [...rows];
      // simple sorter using orderBy specs like [{createdAt:'desc'},{id:'desc'}]
...
export async function run({ prisma, url, params = {} }) {
  // Pull latest event per transportId (deterministic by createdAt, id)
  ...
  // Keep only latest per transportId
  ...
  for (const latest of latestById.values()) {
    ...
    // We also want original dispatch info for received/canceled
    ...
    rowsAll.push({
      ...
      // deltas (only meaningful if received)
  ...
  // Paginate the computed array using the same engine (+1 look-ahead & count)
  const { rows, paging } = await paginateQuery(arrayDelegate(rowsAll), {
    orderBy, page, pageSize, select: { /* passthrough */ createdAt: true, id: true, transportId: true, lane: true, supplierId: true, dispatchQty: true, receiveQty: true, qtyDelta: true, dispatchAmount: true, receiveAmount: true, amountDelta: true, status: true, shade: true, size: true },
    totalMode: 'count'
  });
