...
// /home/bilal-tariq/ab/src/lib/mma/mma.js
// Single-ledger MMA engine (uses Prisma models: Supplier, Raw).
// MMAs are logic-only codes (e.g., "ABS.SLOTS", "PSS.DUMP") validated against a registry.
//
// API (lean):
//   deposit({ mmaCode, supplierId, shade, size, qty, meta? })
//   dispatch({ fromMmaCode, toMmaCode, supplierId, shade, size, qty, meta? })
//   receive({ id, toMmaCode, supplierId, receiveQty?, receiveShade?, meta? })  // completes the SAME row
//   cancel({ id, meta? })
//   onHand({ mmaCode, supplierId?, shade?, size? })
//   stock({ mmaCode, positiveOnly=true })
//   inbound({ mmaCode, status='IN_TRANSIT' })
//   outbound({ mmaCode, status='IN_TRANSIT' })
//   activeSlots({ mmaCode })
//
// Important: receive() verifies supplier (and destination MMA) match the dispatch row.

import { PrismaClient, EdgeStatus, BornAs } from '@prisma/client';

export function createMMA(opts = {}) {
  ...
  async function onHandTx(tx, { mmaCode, supplierId, shade, size }) {
    const depIn = await tx.raw.aggregate({
    ...
    const trIn = await tx.raw.aggregate({
      _sum: { receiveQty: true },
      where: {
    ...
    const trOut = await tx.raw.aggregate({
  ...
  return {
    ...
    async receive({ id, toMmaCode, supplierId, receiveQty, receiveShade, meta }) {
      ...
      return prisma.$transaction(async (tx) => {
        const row = await tx.raw.findUnique({ where: { id } });
        ...
        if (row.status === 'RECEIVED') {
          ...
          if (meta && JSON.stringify(meta) !== JSON.stringify(row.meta ?? null)) {
            await tx.raw.update({ where: { id: row.id }, data: { meta } });
          }
          return row;
        }

        // Verify supplier and destination MMA match the expected receiver
        ...
        const updated = await tx.raw.update({
          where: { id: row.id },
          data: {
            status: EdgeStatus.RECEIVED,
            receiveQty: finalQty,
    ...
    async onHand({ mmaCode, supplierId, shade, size }) {
      assertMma(mmaCode, 'mmaCode');
      return prisma.$transaction((tx) => onHandTx(tx, { mmaCode, supplierId, shade, size }));
    },
    ...
    async inbound({ mmaCode, status = 'IN_TRANSIT' } = {}) {
      ...
      return prisma.raw.findMany({ where, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }] });
    },
...
const _default = createMMA({ registry: defaultRegistry });
