...
// Procurement — Overview (live): KPIs, station×family rollup, supplier leaderboard, recent purchases
...
function makeWhere(filters) {
  ...
  const where = { reason: 'DIRECT' }; // DIRECT = procurement deposits
...
export const load = async ({ url }) => {
  ...
  const leaderboardRaw = [...lbMap.entries()]
    .map(([supplierId, qty]) => ({ supplierId, qty }))
  ...
  const families = ['RAW', 'SCREENED', 'SORTED', 'PRODUCTION']; // show all; most activity is RAW/SCREENED
  ...
  // Recent purchases (latest 25 rows)
