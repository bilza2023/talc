  ...
  <div class="mx-auto max-w-3xl px-4 py-8">
    ...
    {#if form?.success}
      <div class="mb-4 rounded-xl bg-green-600/20 text-green-200 px-4 py-3 ring-1 ring-green-700/40">
        Edge created (ID: {form.edgeId}) → {form.toStation}
      </div>
    ...
    <form method="POST" action="?/dispatch" use:enhance class="space-y-6 bg-[#101721] rounded-2xl p-6 shadow-lg ring-1 ring-[#0f1724]">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div class="flex flex-col gap-2 md:col-span-2">
          <label class="text-sm text-[#a9b3c2]" for="parentBatchId">Parent batch</label>
          ...
          <p class="text-xs text-[#7f8aa3]">You can only dispatch up to the batch’s available (remaining − in-transit committed).</p>
        </div>
