...
class Stock {
  ...
  constructor({
    ...
    this.prisma = prisma2 ?? new PrismaClient();
  ...
  #size(size) {
    return size ?? this.sizeDefault;
  }
  #need(val, name) {
    if (val == null || val === "") throw new Error(`${name} is required`);
  }
  #needPos(qty) {
    ...
    if (isPurchase) {
      ...
      return this.prisma.$transaction(async (tx) => {
        ...
        const pur = await tx.purchase_tbl.create({
          data: {
            docDate: purchase?.docDate ? new Date(purchase.docDate) : /* @__PURE__ */ new Date(),
            supplierId: Number(supplierId),
  ...
  async dispatch({
    ...
    this.#needPos(qty);
    const finalSize = this.#size(size);
    const tid = transportId ?? randomUUID();
    return this.prisma.$transaction(async (tx) => {
      const available = await this.onHand({ mmaCode: fromMmaCode, supplierId, shade, size: finalSize });
      if (Number(available) < Number(qty)) {
        throw new Error(`Insufficient stock at ${fromMmaCode} (available=${available}, requested=${qty})`);
  ...
  async receive({
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (alreadyReceive) return { transportId, receive: alreadyReceive, posting: null };
  ...
  async cancel({ transportId, meta }) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const canceled = await T.findFirst({ where: { transportId, type: "CANCEL" } });
      if (canceled) return { transportId, cancel: canceled, posting: null };
...
export { Stock as S, prisma as p, stock as s };
//# sourceMappingURL=stockEngine-jmqVw6zN.js.map
